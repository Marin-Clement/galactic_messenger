...
//        performLoginOrRegister();
        ...
        try {
            ...
            stompSession.subscribe("/topic/messages/" + username, new StompFrameHandler() {
                ...
                public void handleFrame(StompHeaders headers, Object payload) {
                    byte[] PrivateMessageBytes = (byte[]) payload;
                    PrivateMessage PrivateMessage = deserializePrivateMessage(PrivateMessageBytes);
                    // if the PrivateMessage is from the user, don't print it
                    if (!PrivateMessage.getSender().equals(username) && PrivateMessage.getRecipient().equals(username)) {
                        System.out.println("Received PrivateMessage: " + PrivateMessage.getContent() + " from " + PrivateMessage.getSender());
                    }
                }
            });
            ...
            while (true) {
                String recipient;
                System.out.println("Enter a PrivateMessage to send (or type 'exit' to quit): ");
                ...
                PrivateMessage PrivateMessage = new PrivateMessage(username, recipient, input);
                byte[] PrivateMessageBytes = serializePrivateMessage(PrivateMessage);
                stompSession.send("/app/send-PrivateMessage", PrivateMessageBytes);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void SendPrivatePrivateMessage(String recipient, String content){
        PrivateMessage PrivateMessage = new PrivateMessage(username, recipient, content);
        byte[] bytesPrivateMessage = serializePrivateMessage(PrivateMessage);
        stompSession.send("/app/send-PrivateMessage/" + recipient, bytesPrivateMessage);
    }

    private void SendGroupPrivateMessage(String groupName, String content){
        PrivateMessage PrivateMessage = new PrivateMessage(username, groupName, content);
        byte[] bytesPrivateMessage = serializePrivateMessage(PrivateMessage);
        stompSession.send("/app/send-group-PrivateMessage/" + groupName, bytesPrivateMessage);
    }

    private byte[] serializePrivateMessage(PrivateMessage PrivateMessage) {
        try {
    ...
    private PrivateMessage deserializePrivateMessage(byte[] PrivateMessageBytes) {
        try {
            return objectMapper.readValue(PrivateMessageBytes, PrivateMessage.class);
        } catch (Exception e) {
            e.printStackTrace();
